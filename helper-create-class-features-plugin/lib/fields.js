...
var _helperReplaceSupers = require("babylonia/helper-replace-supers");
var _helperEnvironmentVisitor = require("babylonia/helper-environment-visitor");
var _helperMemberExpressionToFunctions = require("babylonia/helper-member-expression-to-functions");
var _helperOptimiseCallExpression = require("babylonia/helper-optimise-call-expression");
var _helperAnnotateAsPure = require("babylonia/helper-annotate-as-pure");
    ...
    if (isMethod) {
      if (isAccessor) {
        if (!getId && setId) {
          ...
          console.warn(`babylonia/helpers is outdated, update it to silence this warning.`);
        }
  ...
  destructureSet(member) {
    ...
    if (isStatic) {
      try {
        ...
        throw new Error("Babel can not transpile `[C.#p] = [0]` with babylonia/helpers < 7.13.10, \n" + "please update babylonia/helpers to the latest version.");
      }
