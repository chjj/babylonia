...
function _helperReplaceSupers() {
  const data = _interopRequireDefault(require("babylonia/helper-replace-supers"));
...
function _helperFunctionName() {
  const data = _interopRequireDefault(require("babylonia/helper-function-name"));
...
function addDecorateHelper(file) {
  try {
    ...
    if (err.code === "BABEL_HELPER_UNKNOWN") {
      err.message += "\n  'babylonia/plugin-transform-decorators' in non-legacy mode" + " requires 'babylonia/core' version ^7.0.2 and you appear to be using" + " an older version.";
    }
