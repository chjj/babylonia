...
import _getTargets, { prettifyTargets, getInclusionReasons, isRequired } from 'babylonia/helper-compilation-targets';
import * as _babel from 'babylonia/core';
...
const presetEnvSilentDebugHeader = "#__secret_key__babylonia/preset-env__don't_log_debug_header_and_resolved_targets";
      ...
      pre(file) {
        ...
        if (runtimeName) {
          if (file.get("runtimeHelpersModuleName") && file.get("runtimeHelpersModuleName") !== runtimeName) {
            console.warn(`Two different polyfill providers` + ` (${file.get("runtimeHelpersModuleProvider")}` + ` and ${providerName}) are trying to define two` + ` conflicting babylonia/runtime alternatives:` + ` ${file.get("runtimeHelpersModuleName")} and ${runtimeName}.` + ` The second one will be ignored.`);
          } else {
