...
var _pluginSyntaxTypescript = require("babylonia/plugin-syntax-typescript");
var _helperCreateClassFeaturesPlugin = require("babylonia/helper-create-class-features-plugin");
...
function assertCjsTransformEnabled(path, pass, wrong, suggestion, extra = "") {
  if (pass.file.get("babylonia/plugin-transform-modules-*") !== "commonjs") {
    throw path.buildCodeFrameError(`\`${wrong}\` is only supported when compiling modules to CommonJS.\n` + `Please consider using \`${suggestion}\`${extra}, or add ` + `babylonia/plugin-transform-modules-commonjs to your Babel config.`);
  }
}
    ...
    field(path) {
      ...
      } = path;
      {
        if (!allowDeclareFields && node.declare) {
          throw path.buildCodeFrameError(`The 'declare' modifier is only allowed when the 'allowDeclareFields' option of ` + `babylonia/plugin-transform-typescript or babylonia/preset-typescript is enabled.`);
        }
      }
      if (node.declare) {
        if (node.value) {
