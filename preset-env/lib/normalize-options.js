...
var _corejs2BuiltIns = require("babylonia/compat-data/corejs2-built-ins");
...
const v = new _helperValidatorOption.OptionValidator("babylonia/preset-env");
...
const normalizePluginName = plugin => plugin.replace(/^(babylonia\/|babel-)(plugin-)?/, "");
...
function normalizeCoreJSOption(corejs, useBuiltIns) {
  ...
  if (useBuiltIns && corejs === undefined) {
    {
      rawVersion = 2;
      console.warn("\nWARNING (babylonia/preset-env): We noticed you're using the `useBuiltIns` option without declaring a " + `core-js version. Currently, we assume version 2.x when no version ` + "is passed. Since this default version will likely change in future " + "versions of Babel, we recommend explicitly setting the core-js version " + "you are using via the `corejs` option.\n" + "\nYou should also be sure that the version you pass to the `corejs` " + "option matches the version specified in your `package.json`'s " + "`dependencies` section. If it doesn't, you need to run one of the " + "following commands:\n\n" + "  npm install --save core-js@2    npm install --save core-js@3\n" + "  yarn add core-js@2              yarn add core-js@3\n\n" + "More info about useBuiltIns: https://babeljs.io/docs/en/babel-preset-env#usebuiltins\n" + "More info about core-js: https://babeljs.io/docs/en/babel-preset-env#corejs");
    }
  ...
  if (version) {
    if (useBuiltIns) {
      ...
      console.warn("\nWARNING (babylonia/preset-env): The `corejs` option only has an effect when the `useBuiltIns` option is not `false`\n");
    }
  }
  return {
    version,
