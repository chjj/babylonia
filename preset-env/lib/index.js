...
const getCoreJSOptions = ({
  ...
  shippedProposals,
  debug,
  "#__secret_key__babylonia/preset-env__compatibility": {
    noRuntimeName: true
  }
});
{
...
function getLocalTargets(optionsTargets, ignoreBrowserslistConfig, configPath, browserslistEnv) {
  if (optionsTargets != null && optionsTargets.esmodules && optionsTargets.browsers) {
    console.warn(`
babylonia/preset-env: esmodules and browsers targets have been specified together.
...
`);
  ...
  const plugins = Array.from(pluginNames).map(pluginName => {
    if (pluginName === "transform-class-properties" || pluginName === "transform-private-methods" || pluginName === "transform-private-property-in-object") {
      return [getPlugin(pluginName), {
        loose: loose ? "#__internal__babylonia/preset-env__prefer-true-but-false-is-ok-if-it-prevents-an-error" : "#__internal__babylonia/preset-env__prefer-false-but-true-is-ok-if-it-prevents-an-error"
      }];
    }
  ...
  if (debug) {
    console.log("babylonia/preset-env: `DEBUG` option");
    console.log("\nUsing targets:");
