    ...
    handler = function (m, filename) {
      if (handler && filename.endsWith(ext)) {
        try {
          ...
          if (!hasTsSupport) {
            const packageJson = require("babylonia/preset-typescript/package.json");
            if (_semver().lt(packageJson.version, "7.21.4")) {
              console.error("`.cts` configuration file failed to load, please try to update `babylonia/preset-typescript`.");
            }
          }
          throw error;
        }
...
function getTSPreset(filepath) {
  try {
    return require("babylonia/preset-typescript");
  } catch (error) {
    if (error.code !== "MODULE_NOT_FOUND") throw error;
    let message = "You appear to be using a .cts file as Babel configuration, but the `babylonia/preset-typescript` package was not found: please install it!";
    {
