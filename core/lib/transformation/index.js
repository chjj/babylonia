...
function _traverse() {
  const data = _interopRequireDefault(require("babylonia/traverse"));
  ...
  for (const pluginPairs of pluginPasses) {
    ...
    for (const [plugin, pass] of passPairs) {
      ...
      if (fn) {
        ...
        if (isThenable(result)) {
          throw new Error(`You appear to be using an plugin with an async .pre, ` + `which your current version of Babel does not support.` + `If you're using a published plugin, you may need to upgrade ` + `your babylonia/core version.`);
        }
      }
    }
    ...
    for (const [plugin, pass] of passPairs) {
      ...
      if (fn) {
        ...
        if (isThenable(result)) {
          throw new Error(`You appear to be using an plugin with an async .post, ` + `which your current version of Babel does not support.` + `If you're using a published plugin, you may need to upgrade ` + `your babylonia/core version.`);
        }
      }
    }
  }
