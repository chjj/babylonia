...
function _generator() {
  const data = require("babylonia/generator");
  _generator = function () {
    return data;
  };
  return data;
...
function generateCode(pluginPasses, file) {
  ...
  if (results.length === 0) {
    ...
    if (typeof result.then === "function") {
      throw new Error(`You appear to be using an async codegen plugin, ` + `which your current version of Babel does not support. ` + `If you're using a published plugin, ` + `you may need to upgrade your babylonia/core version.`);
    }
  } else {
