...
var _helperSimpleAccess = require("babylonia/helper-simple-access");
const {
...
const rewriteReferencesVisitor = {
  ReferencedIdentifier(path) {
    ...
    if (importData) {
      if (isInType(path)) {
        throw path.buildCodeFrameError(`Cannot transform the imported binding "${localName}" since it's also used in a type annotation. ` + `Please strip type annotations using babylonia/preset-typescript or babylonia/preset-flow.`);
      }
