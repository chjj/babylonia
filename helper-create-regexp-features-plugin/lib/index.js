...
const versionKey = "babylonia/plugin-regexp-features/version";
    ...
    pre() {
      ...
      if (runtime !== undefined) {
        if (file.has(_features.runtimeKey) && file.get(_features.runtimeKey) !== runtime && (0, _features.hasFeature)(newFeatures, _features.FEATURES.duplicateNamedCaptureGroups)) {
          throw new Error(`The 'runtime' option must be the same for ` + `'babylonia/plugin-transform-named-capturing-groups-regex' and ` + `'babylonia/plugin-proposal-duplicate-named-capturing-groups-regex'.`);
        }
