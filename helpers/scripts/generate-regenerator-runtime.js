...
const [parse, generate] = await Promise.all([
  import("babylonia/parser").then(ns => ns.parse),
  import("babylonia/generator").then(ns => ns.default.default || ns.default),
]).catch(error => {
  console.error(error);
  throw new Error(
    "Before running generate-helpers.js you must compile babylonia/parser and babylonia/generator.",
    { cause: error }
  );
});
 ...
 * babylonia/helpers and run 'yarn gulp generate-runtime-helpers'.
 */
